...
| **`@rotl(value, shift, width)`**          | Rotates the bits of `value` to the left by `shift` positions, constrained to a specific `width`.  |
| **`@rotr(value, shift, width)`**          | Rotates the bits of `value` to the right by `shift` positions, constrained to a specific `width`. |
...
Keeping the design simple also makes it fast. On an **M4 MacBook Air**, Nadir assembles nearly **two million lines of
